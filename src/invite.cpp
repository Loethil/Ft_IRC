...
void Server::invite(Clients *client, std::istringstream &lineStream)
{
    ...
    if (lineStream >> nickname >> channelName)
    {
        if(_Channel[channelName])
        {
            ...
            if (chanIt != _Channel.end())
            {
                ...
                if (invitedClient != NULL)
                {
                    ...
                    std::string successMsg = ":" + client->getNickname() + "!" + client->getUsername() + "@I.R.SIUSIU NOTICE " + channelName + " :You invited " + nickname + " to " + channelName + "\n";
        ...
        else
        {
            ...
            std::string errMsg = RED "No such channel: " + channelName + RESET "\n";
            send(client->getSocket(), errMsg.c_str(), errMsg.length(), 0);
        }
    }
}
